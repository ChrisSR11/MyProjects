...
1. The dictionary implementation given assumes each word is a lowercase letter 
   ...
   class Node {
    // Your answer here
   Node* letters[NUM_CHARS + newChars];
   bool isWord = false;
   Node()
   {
        int totalChars = NUM_CHARS + newChars
...
4.  Why is the prefix tree better than a binary search tree for implementing 
    ...
    > Prefix tree has an array of nodes making operation on this tree more efficient (better in time and space complexity)
> compared to a binary tree that can store one data type

5.  Suppose you do not use `IsPrefix` in your `SolveBoard` implementation.  
    How would that affect the program?

    > if not use then we will be doing unnecessary exploring prefixes that we know will not be a word
